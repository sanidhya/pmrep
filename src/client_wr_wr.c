...
void *run_bench(void *arg)
{
    ...
    uint64_t i;
    struct timespec start_t, end_t, mid_t1;
    ...
    uint64_t *timediff = mem_alloc_pgalign(sizeof(uint64_t) * opt.iterations, "");
    uint64_t *timediff1 = mem_alloc_pgalign(sizeof(uint64_t) * opt.iterations, "");
    uint64_t *timediff2 = mem_alloc_pgalign(sizeof(uint64_t) * opt.iterations, "");
    int cnt = 0;
    ...
    for (i = 0; i < opt.iterations; i++) {
        count = (count + 1) % 255;
        memset(buf + start, count, opt.buffer_size);
        ...
        clock_gettime(CLOCK_MONOTONIC, &mid_t1);
        rwr_node = poll_recv_cq_client(&pctx, 0, tid);
        get_and_post_recv_wr(&pctx, rwr_node, 0);
        timediff[cnt] = get_time_diff(mid_t1, end_t);
        timediff1[cnt] = get_time_diff(start_t, end_t);
        timediff2[cnt++] = get_time_diff(start_t, mid_t1);
    }
    clock_gettime(CLOCK_MONOTONIC, &end_t);

#if 0
    double avg = 0.0;
    for (i = 2; i <= opt.iterations; ++i) {
        uint64_t td = timediff1[i-1] - timediff[i-1];
        avg += (double)td;
        printf("count: %lu, write time: %lu, recv diff: %lu, avg: %lf, last: (sm:%lu bg:%lu)\n",
               i, timediff2[i-1], td, avg / (double)(i-1), timediff[i-1], timediff1[i-1]);
    }
#endif
