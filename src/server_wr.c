...
void handler(int sig)
{
    ...
    for (i = 1; i <= 10000; ++i) {
        avg += timediff[i-1];
        fprintf(stderr, "count: %d, time: %lu, avg: %lf\n",
                i, timediff[i-1], avg / (double)i);
    }
#endif
    exit(0);
}
...
static inline void calculate_avg(uint64_t *diff, int count)
{
    double avg = 0.0;
    int i;
    for (i = 1; i < count; ++i) {
